...
class TrainDP3Workspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        self.cfg = cfg
        self._output_dir = output_dir       #保存路径
        ...
        # configure model
        #使用配置中的 policy来初始化 model
        self.model: DP3 = hydra.utils.instantiate(cfg.policy)     #：DP3为类型注解
    ...
    def run(self):
        ...
        WANDB = False
        #如果为debug模式，训练参数会较小，用于验证代码逻辑是否有问题
        ...
        lr_scheduler = get_scheduler(
            ...
            num_warmup_steps=cfg.training.lr_warmup_steps,       #设置训练预热
            num_training_steps=(
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            #创建一个记录loss的list
            train_losses = list()
            #创建一个进度条tqdm对象赋值给tepoch  
            #参数: dataloader desc:进度条备注  leave:结束后是否保留，如果不保留，则删除    
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}",   
                ...
                for batch_idx, batch in enumerate(tepoch):    
                    ...
                    # update ema
                    if cfg.training.use_ema:
                        ema.step(self.model)  #使用当前模型的参数更新ema参数
                    t1_4 = time.time()
                    # logging
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000):
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(f'./policy/3D-Diffusion-Policy/checkpoints/{cfg.task.name}_{cfg.head_camera_type}_{cfg.expert_data_num}_{cfg.training.seed}/{checkpoint_num}.ckpt')
        else:
            ckpt_file = pathlib.Path(f'./policy/3D-Diffusion-Policy/checkpoints/{cfg.task.name}_{cfg.head_camera_type}_{cfg.expert_data_num}_w_rgb_{cfg.training.seed}/{checkpoint_num}.ckpt')
